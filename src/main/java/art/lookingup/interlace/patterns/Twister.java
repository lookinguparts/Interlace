...
/**
 * Class for using twist angle to twist point locations and update the model.
 */
public class Twister extends LXPattern {

  CompoundParameter angle1 = new CompoundParameter("angle1", 0, 0, 360);
  CompoundParameter angle2 = new CompoundParameter("angle2", 0, 0, 360);
  CompoundParameter angle3 = new CompoundParameter("angle3", 0, 0, 360);
  ...
  public Twister(LX lx) {
    super(lx);
    addParameter("angle1", angle1);
    addParameter("angle2", angle2);
    addParameter("angle3", angle3);
    angle1.addListener((p) -> {
      updatePointPositions(0, ((float)Math.PI * 2f * p.getValuef()/360f));
    });
    angle2.addListener((p) -> {
      updatePointPositions(1, ((float)Math.PI * 2f * p.getValuef()/360f));
    });
    angle3.addListener((p) -> {
      updatePointPositions(2, ((float)Math.PI * 2f * p.getValuef()/360f));
    });
  ...
  protected void run(double deltaMs) {
    // Do nothing.  This pattern is just for rotating the hyperboloid.
  }
}
